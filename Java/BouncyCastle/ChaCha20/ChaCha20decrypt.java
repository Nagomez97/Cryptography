...
class CHACHAdecrypt {
	...
	public static void main(String[] args) {
		...
		if(args.length < 2){
			System.out.print("Usage: java CHACHAdecrypt [cipherdata] [secret key]\n");
			System.exit(0);
		}
		...
		try {
			...
			Cipher chachaCipherForDecryption = Cipher.getInstance(transformationString, "BC");
			...
			chachaCipherForDecryption.init(Cipher.DECRYPT_MODE, secretKey, new IvParameterSpec(iv));
			
			// Step 4: decrypt the message.
			byte[] decrypted = chachaCipherForDecryption.doFinal(encrypted);
